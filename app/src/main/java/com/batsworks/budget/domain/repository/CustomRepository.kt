...
class CustomRepository<T>(
	...
	fun findAfterId(id: String): Task<QuerySnapshot> {
		val query = db.collection(collection).whereEqualTo("id", id).orderBy("id")
		return query.startAfter(id).get()
	}
	...
	suspend fun idNotIn(ids: List<String>): Task<QuerySnapshot> {
		return db.collection(collection).whereNotIn("id", ids).get()
	}
}