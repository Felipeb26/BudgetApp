    ...
    private fun submit() {
        viewModelScope.launch {
            try {
                ...
                user = user.withName(state.nome?:user.nome)
                    .withEmail(state.email?:user.email)
                    .withPhone(state.telefone?:user.phone)
                    .withPassword(state.password?:user.password.toString())
                localRepository.save(user)
