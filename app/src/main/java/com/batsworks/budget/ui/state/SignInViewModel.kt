            ...
            repository.findByParam(
                ...
                .addOnSuccessListener { documents ->
                    for (document in documents) {
                        viewModelScope.launch {
                            val userDTO = querySnapshotToEntity(document.data)
                            if (userDTO.email == state.email) {
                                val user =
                                    localRepository.findByLogin(state.email, state.password.toInt())
                                if (user == null) {
                                    localRepository.save(querySnapshotToEntity(document.data))
                                }
